...
#     Своё фото назови musya.jpg, чтобы оно встало первым.
#     Сколько фото положишь — столько и будет на сайте:
...
# Старые member-*.jpg убираем: иначе при уменьшении числа фото
# на сайте останутся висеть лишние карточки.
rm -f "$OUT"/member-*.jpg
